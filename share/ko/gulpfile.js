...
// Config
var requireJsRuntimeConfig = vm.runInNewContext(fs.readFileSync(source + '/app/require.config.js') + '; require;'),
    requireJsOptimizerConfig = merge(requireJsRuntimeConfig, {
        ...
        bundles: {
            ...
            tetris: ['components/tetris/tetris', 'components/tetris/round', 'components/tetris/block'],
...
gulp.task('css', function () {
    var bowerCss = gulp.src(source + '/bower_modules/components-bootstrap/css/bootstrap.min.css')
            .pipe(replace(/url\((')?\.\.\/fonts\//g, 'url($1fonts/')),
        appCss = gulp.src(source + '/css/*.css'),
        combinedCss = es.concat(bowerCss, appCss).pipe(concat('css.css')),
        fontFiles = gulp.src(source + '/bower_modules/components-bootstrap/fonts/*', { base: './src/bower_modules/components-bootstrap/' });
...
gulp.task('serve:dist', ['default'], function() {
    return connect.server({ root: destination });
});
...
gulp.task('default', ['html', 'js', 'css'], function(callback) {
    callback();
    console.log('\nPlaced optimized files in ' + chalk.magenta(destination + "\n"));
});
